...
from argonathdb.typing import FloatMatrix
...
def split_chunks(
    ...
    #Normalize the sentence embeddings to norm score
    X = sentence_embeddings.astype(np.float32)
    ...
    if not np.any(np.linalg.norm(X-discourse[np.newaxis, :],axis=1) <= np.finfo(X.dtype).eps):
         X = X - np.outer(X @ discourse,discourse)
         X = X / np.linalg.norm(X,axis=1,keepdims=True)
