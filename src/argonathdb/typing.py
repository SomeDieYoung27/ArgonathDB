...
from argonathdb import RAGLiteConfig
...
class NumpyArray(TypeDecorator[np.ndarray[Any,np.dtype[np.floating[Any]]]]):
      ...
      def process_bind_param(
                  self,
                  value : np.ndarray[Any,np.dtype[np.floating[Any]]] | None,dialect : Dialect
...
class PickedObject(TypeDecorator[object]):
     ...
     def process_result_value(self,value : bytes | None,dialect : Dialect) -> object | None:
            ...
            return pickle.loads(value, fix_imports=False)
...
class HalfVec(UserDefinedType[FloatVector]):
     ...
     class comparator_factory(HalfVecComparatorMixin):
          ...
