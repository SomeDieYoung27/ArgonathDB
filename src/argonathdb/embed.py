...
from argonathdb.config import RAGLiteConfig
from argonathdb.lazy_llama import LLAMA_POOLING_TYPE_NONE, Llama
from argonathdb.litellm import LlamaCppPythonLLM
from argonathdb.typing import FloatMatrix, IntVector


def embed_sentences_with_late_chunking(  # noqa: PLR0915
    ...
    while content_start_index < len(sentences):
        segment_start_index, segment_end_index = create_segment(
...
def embed_string_batch(string_batch: list[str], *, config: RAGLiteConfig) -> FloatMatrix:
...
def embed_strings(strings : list[str], *,config : RAGLiteConfig | None = None) -> FloatMatrix:
     ...
     batch_embeddings = [
         embed_string_batch(strings[i : i + batch_size], config=config)
...
def embed_sentences_with_windowing(
    sentences: list[str], *, config: RAGLiteConfig | None = None
) -> FloatMatrix:
...
def embed_sentences(sentences: list[str], *, config: RAGLiteConfig | None = None) -> FloatMatrix:
     ...
     if sentence_embedding_type(config=config) == "late_chunking":
         sentence_embeddings = embed_sentences_with_late_chunking(sentences,config=config)

     else:
         sentence_embeddings = embed_sentences_with_windowing(sentences,config=config)
